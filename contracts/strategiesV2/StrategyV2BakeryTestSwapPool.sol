...
contract StrategyV2BakeryTestSwapPool is IStrategyV2SwapPool {
    ...
    IBakeryFarmPool public constant farmpool = IBakeryFarmPool(0x20eC291bB8459b6145317E7126532CE7EcE5056f);
    ...
    function getPending(uint256 _poolId) external override view returns (uint256 rewards) {
        rewards = farmpool.pendingBake(address(_poolId), address(this));
    }
    ...
    function deposit(uint256 _poolId, bool _autoPool)
        ...
        if(amountA > 0 && amountB > 0) {
            ...
            if(liquidity > 0 && _autoPool) {
                ...
                farmpool.deposit(address(_poolId), liquidity);
            }
        }
    ...
    function withdraw(uint256 _poolId, uint256 _liquidity, bool _autoPool)
        ...
        if(_autoPool) {
            farmpool.withdraw(address(_poolId), _liquidity);
        }
        address lpToken = getDepositToken(_poolId);
        (address tokenA, address tokenB) = getToken01(lpToken);
    ...
    function claim(uint256 _poolId) 
        ...
        farmpool.deposit(address(_poolId), 0);
