...
import "../../interfaces/IJSwapPool.sol";
import "../../interfaces/IJSwapSwapPool.sol";

import "../interfaces/IStrategyV2SwapPool.sol";
import "../utils/TenMath.sol";

// https://app.jswap.finance/#/swap

// Connecting to third party swap for pool lptoken
contract StrategyV2JSwapPool is IStrategyV2SwapPool {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapV2Factory public constant factory = IUniswapV2Factory(0xd654CbF99F2907F06c88399AE123606121247D5C);
    IUniswapV2Router02 public constant router = IUniswapV2Router02(0x069A306A638ac9d3a68a6BD8BE898774C073DCb3);
    ...
    function getPending(uint256 _poolId) external override view returns (uint256 rewards) {
        rewards = farmpool.pendingJf(_poolId, address(this));
    }
    ...
    function emergencyWithdraw(uint256 _poolId)
        external onlyStrategy returns (uint256 rewards) {
        farmpool.emergencyWithdraw(_poolId);
        address lpToken = getDepositToken(_poolId);
        (address tokenA, address tokenB) = getToken01(lpToken);
        _safeTransferAll(lpToken, strategy);
        _safeTransferAll(tokenA, strategy);
    ...
    function extraRewards()
        ...
        token = swappool.jfToken();
        rewards = swappool.pending(address(this));
        if(rewards > 0) {
            swappool.withdraw();
